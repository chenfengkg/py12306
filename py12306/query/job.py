...
class Job:
    ...
    def query_by_date(self, date):
        ...
        if Config.is_cdn_enabled() and Cdn().is_ready:
            self.is_cdn = True
            return self.query.session.cdn_request(url, timeout=self.query_time_out, allow_redirects=True)#改成True避免moved temporaily
            #return self.query.session.cdn_request(url, timeout=self.query_time_out, allow_redirects=False)
        self.is_cdn = False
        return self.query.session.get(url, timeout=self.query_time_out, allow_redirects=True)#改成True避免moved temporaily
